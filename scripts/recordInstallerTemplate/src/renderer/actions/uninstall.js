...
export default async function(config) {
    ...
    for (const channel of channels) {
        lognewline(`Removing ReCord from Discord ${channel}...`);
        try {
            await runCli(["-uninstall", "-branch", channel]);
            log(`\u2705 ReCord removed successfully on ${channel}`);
            progress.set(progress.value + progressPerChannel);
        }
        catch (err) {
            log(`\u274c Failed to uninstall on ${channel}: ${err.message}`);
            return fail();
        }
    }
